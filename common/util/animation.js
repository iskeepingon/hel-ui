...
export function animation(easing, duration, process) {
  return parseEasings(easing, duration)(process);
}

export function doAnimate(Config) {
  let rafId = 0;
  const defaultConfig = {
    initValue: 0,
    targetValue: 0,
  ...
  const initValue = Number(customConfig.initValue);
  const targetValue = Number(customConfig.targetValue);
  const duration = Number(customConfig.duration);
  if (duration <= 0) return;
  if (initValue === targetValue) return;
  const startTime = Date.now();
  let currentNumber = 0;
  const diff = targetValue - initValue;
  let progress = 0;
  ...
  function doNow() {
    ...
    if (progress > 1) {
      is.fnc(customConfig.update) && customConfig.update(targetValue);
      return;
    }
    currentNumber = initValue + (diff * animation(customConfig.easing, duration, progress));
    is.fnc(customConfig.update) && customConfig.update(currentNumber);
    rafId = raf(doNow);
  }
  return doNow();
}
